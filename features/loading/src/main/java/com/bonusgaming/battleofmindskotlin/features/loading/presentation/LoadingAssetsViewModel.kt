...
import com.bonusgaming.battleofmindskotlin.features.loading.domain.usecases.DownloadStickerUseCase
import com.bonusgaming.battleofmindskotlin.features.loading.domain.usecases.DownloadUrlsUseCase
import com.bonusgaming.battleofmindskotlin.features.loading.domain.usecases.GetNextFragmentStateUseCase
import com.bonusgaming.battleofmindskotlin.features.loading.domain.usecases.SaveStickerUseCase
import com.bonusgaming.battleofmindskotlin.features.loading.domain.usecases.GetSavedStickersListUseCase
...
private const val DELAY_MILLI = 2000L

@PerFeature
class LoadingAssetsViewModel
@Inject constructor
(private val downloadUrls: DownloadUrlsUseCase,
 private val downloadSticker: DownloadStickerUseCase,
 private val nextState: GetNextFragmentStateUseCase,
 private val saveStickers: SaveStickerUseCase,
 private val savedStickers: GetSavedStickersListUseCase,
 private val resources: Resources) : ViewModel() {
    ...
    private fun nextFragmentState() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            withContext(Dispatchers.Main) {
                _loadSceneLiveData.value = nextState.execute()
            }
        }
    }

    private fun startDownloadUrls() {
        val disposable = downloadUrls.execute({
    ...
    private fun proceedResult(list: List<UrlSticker>) {
        ...
        val perProgress = when (list.size) {
            0 -> MAXIMUM_PROGRESS.toFloat()
            else -> (MAXIMUM_PROGRESS / list.size).toFloat()
        }
        ...
        fun saveSticker(sticker: Sticker, bitmap: Bitmap) {
            viewModelScope.launch(Dispatchers.IO) {
                ...
                saveStickers.saveToDisk(sticker, bitmap)
            }
        }
        ...
        fun updateProgress() {
            ...
            if (progressRounded == MAXIMUM_PROGRESS) {
...
//                databaseHashList.forEach { println(" $it ") }
                ...
                val onException: (url: String) -> Unit = {
                    ...
                    downloadSticker.retryDownloadWithDelay(namedUrlSticker)
                }
        ...
        viewModelScope.launch(Dispatchers.IO)
        {
            val databaseHashList = savedStickers.execute()
            withContext(Dispatchers.Main) {
